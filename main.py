...
def main(_):
  ...
  server = tf.train.Server(ClusterSpecification.CLUSTER_SPEC, job_name='worker', task_index=FLAGS.index)
  ...
  with tf.device(tf.train.replica_device_setter(cluster=ClusterSpecification.CLUSTER_SPEC)):
  ...
  with tf.train.MonitoredTrainingSession(master=server.target, is_chief=(FLAGS.index == 0)) as sess:
    ...
    for step in xrange(int(num_epochs * train_size) // BATCH_SIZE):
      ...
      if step % EVAL_FREQUENCY == 0:
        ...
        message = "[WORKER:%d]\n" %FLAGS.index
        message += "Step %d (epoch %.2f), %.1f ms\n" % (step, float(step) * BATCH_SIZE / train_size, 1000 * elapsed_time / EVAL_FREQUENCY)
        message += "Minibatch loss: %.3f, learning rate: %.6f\n" % (l, lr)
        message += "Minibatch error: %.1f%%\n" % error_rate(predictions, batch_labels)
        message += "Validation error: %.1f%%" % error_rate(eval_in_batches(validation_data, sess), validation_labels)
        print(message)
        sys.stdout.flush()
